  ...
  shellHook = ''
    ...
    build() {
      cargo test --no-run --target ${hostPlatform.config} --features icecap \
        ...
        -j $NIX_BUILD_CORES \
        --target-dir build/veracruz-test/target --manifest-path ../veracruz-test/Cargo.toml \
        $@ \
      && install
    }

    install() {
      d=build/veracruz-test/target/aarch64-unknown-linux-gnu/${if debug then "debug" else "release"}/deps
      f="$(find $d -executable -type f -printf "%T@ %p\n" \
        | sort -n \
        | tail -n 1 \
        | cut -d ' ' -f 2 \
      )"
      mkdir -p build/veracruz-test/out
      mv "$f" build/veracruz-test/out/veracruz-test
    }
  '';
